...
- `slug` — ссылка, куда ведет карточка. Это поле можно менять отдельно для каждого проекта. Например: `/projects/motor-skin/`.
...
Важно: ссылка в `slug` должна совпадать с адресом существующей страницы. Если страницы пока нет, карточку можно временно вести на уже существующую страницу `/projects/motor-skin/`.
